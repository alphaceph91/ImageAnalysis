...
EDGE_THRESHOLD = 100  #Edge count threshold to classify poor quality. Adjust the edge count if required
...
def apply_sobel(image: np.ndarray) -> np.ndarray:
    ...
    Returns:
        Sobel edge map as a numpy array.
    """
    sobel_x = cv2.Sobel(image, cv2.CV_64F, 1, 0, ksize=3)
    sobel_y = cv2.Sobel(image, cv2.CV_64F, 0, 1, ksize=3)
    sobel_combined = cv2.magnitude(sobel_x, sobel_y)
    #converting to 8-bit unsigned integer
    sobel_edge = np.uint8(np.absolute(sobel_combined))
    return sobel_edge

def process_image(image: np.ndarray) -> np.ndarray:
    """
    Process a single image for Sobel edge detection by converting it to grayscale and applying the Sobel operator
    
    Args:
        image: Input image as a numpy array
    
    Returns:
        Sobel edge map (grayscale image)
    """
...
def count_edges(image: np.ndarray) -> int:
    """
    Count the number of edge pixels using the Sobel edge detector
...
def save_example_edge_image(example_image_path: Path, output_dir: Path) -> None:
    """
    Process an example image and save the resulting Sobel edge map as a grayscale image
    ...
    plt.title("Sobel Edge Detection Example")
    plt.axis("off")
    output_file = output_dir / f"sobel_edges_{example_image_path.name}.png"
    plt.savefig(str(output_file))
    plt.close()
    logging.info(f"Example Sobel edge image saved to {output_file}")
...
def plot_and_save_histogram(edge_counts: list, num_good: int, num_poor: int, output_path: Path) -> None:
    ...
    plt.hist(edge_counts, bins=50, color="blue", edgecolor="black")
    plt.title("Distribution of Edge Counts in Images (Sobel)")
    ...
    plt.close()
    logging.info(f"Sobel histogram saved to {output_path}")
...
def process_images(image_dir: Path, output_dir: Path, edge_threshold: int = EDGE_THRESHOLD):
    """
    Process all images in a directory for Sobel edge detection
...
def process_single_image(image_path: Path, output_dir: Path) -> None:
    """
    Process a single image for Sobel edge detection
    
    This function loads one image (grayscale or RGB), computes its Sobel edge map,
    ...
    cv2.imwrite(str(output_image_path), processed)
    logging.info(f"Processed Sobel edge image saved to {output_image_path}")

def main():
    parser = argparse.ArgumentParser(description="Sobel Edge Detection Script")
    ...
    if args.image:
        ...
        histogram_path = output_dir / "sobel_histogram.png"
        ...
        df["Is_Poor_Quality"] = df["Edge_Count"] < EDGE_THRESHOLD
        df.sort_values(by="Image_Name", inplace=True)
        csv_path = output_dir / "sobel_analysis.csv"
